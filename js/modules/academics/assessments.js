...
function renderGrid() {
    ...
    grid.querySelectorAll('[data-open-entry]').forEach(btn => {
        btn.addEventListener('click', () => {
            if (window.navigateTo) window.navigateTo('marks-entry');
        });
    });
}
...
function wireModal(container) {
    container.querySelector('#as-modal-close')?.addEventListener('click', closeModal);
    container.querySelector('#as-form-cancel')?.addEventListener('click', closeModal);
    container.querySelector('#as-modal-overlay')?.addEventListener('click', (e) => {
        if (e.target.id === 'as-modal-overlay') closeModal();
    });
...
export { renderAssessments, destroyAssessments };
